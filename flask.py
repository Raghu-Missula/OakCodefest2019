...
def comparison(x_array_mn, y_array_mn, x_array_avg, y_array_avg):
    ...
    for idx in range(len(x_array_mn)):
        ...
        if y_avg == y_val:
            continue
        elif (y_val == 0 and y_avg > 0) or (y_avg/y_val) > 1.25:
        ...
        elif (y_avg/y_val) < 0.8:
            in_time = "%d hours, %d minutes" % (int(x_val//100), int(x_val%100*60/100))
            try:
                if output[-1][36] == "d":
                    output.append('<div class="alert text-center alert-danger"> <strong>Hey!</strong> Consumption still out of range at <strong>%s</strong> </div>' % in_time)
                elif output[-1][36] == "w" and output[-2][36] == "w":
                    output.append('<div class="alert text-center alert-danger"> <strong>Hey!</strong> Consumption still out of range at <strong>%s</strong> </div>' % in_time)
                else:
                    output.append('<div class="alert text-center alert-warning"> <strong>Uh-oh!</strong> Power usage spiked at <strong>%s</strong> </div>' % in_time)
            except IndexError:
                output.append('<div class="alert text-center alert-warning"> <strong>Uh-oh!</strong> Power usage spiked at <strong>%s</strong> </div>' % in_time)
    #return str(output)
